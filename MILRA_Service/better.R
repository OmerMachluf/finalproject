...
while (TRUE) {
    isTrainMode = try(read.csv("C:\\Users\\shayl\\new\\temp\\mode.csv", as.is = T))
    ...
    if (isTrainMode[1, 1] == "-") {
        ...
        if (isTrainMode[1, 1]) {
            ...
            if (nrow(x) > 0) {
                ...
                # Get the difrences
                try(write(mean(treePred == testingTargetVector), "C:\\Users\\shayl\\new\\temp\\prediction.txt"))
            ...
            if (nrow(x) > 0) {
                ...
                # Get the difrences
                try(write(mean(treePred == arrPositiveClassificationVector), "C:\\Users\\shayl\\new\\temp\\prediction.txt"))
